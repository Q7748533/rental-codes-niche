...
function parseSlugs(combinedSlug: string) {
  const brands = ['hertz', 'avis', 'enterprise', 'national', 'budget', 'alamo', 'dollar', 'thrifty', 'payless'];
  let brandSlug = '';
  let companySlug = '';

  for (const b of brands) {
    if (combinedSlug.startsWith(`${b}-`)) {
      brandSlug = b;
      companySlug = combinedSlug.replace(`${b}-`, '');
      break;
    }
  }
  return { brandSlug, companySlug };
}
      ...
      <main className="max-w-4xl mx-auto px-4 py-12">
        ...
        <nav className="text-sm text-gray-500 mb-6" aria-label="Breadcrumb">
          <ol className="flex items-center space-x-2">
            ...
            <li className="text-gray-400">/</li>
            <li className="text-gray-900 font-medium">{codeData.company.name}</li>
          </ol>
        </nav>
